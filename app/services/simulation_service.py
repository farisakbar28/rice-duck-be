...
class DSSService:
    ...
    def simulate(
        ...
        # 2. Density Engine
        density = compute_density(payload.duck_count, payload.land_area_are, planting_system.k_max_are)
        p_over = density["P_over"]
        p_under = density["P_under"]
        density_status = density["density_status"]

        # Timeline logic
        d_masuk_bebek = payload.planting_date + timedelta(days=variety.hst_masuk)
        d_tarik_bebek = payload.planting_date + timedelta(days=variety.hst_heading)
        t_active = variety.hst_heading - variety.hst_masuk

        # 3. Survival Engine
        n_survive = compute_surviving_ducks(payload.duck_count, r_age, p_over)

        # 4. Yield Engine
        yield_are_predict = compute_yield_components(p_under, p_over, r_age, planting_system.f_yield, 1.0)
        yield_total_predict = yield_are_predict * payload.land_area_are

        # 5. Cost & Material Engine
        # Base assumed needs per are
        n_need_are = 1.15 * payload.land_area_are
        p_need_are = 0.36 * payload.land_area_are
        k_need_are = 0.60 * payload.land_area_are
        ...
        cost_labor_total = compute_labor_costs(payload.land_area_are, payload.duck_count, p_over, r_age)
        cost_infra = compute_infrastructure(payload.duck_count, payload.land_area_are)
        valuation_weed_eco = compute_ecology_weed(cost_labor_total, density["d"], p_over)

        cost_duck_buy = payload.duck_count * (payload.duck_buy_price_rp_per_duck or 25000.0)
        cost_pesticide = 6440.0 # Standard local estimate from doc logic
        ...
        revenue_gabah = yield_total_predict * 6000.0
        revenue_duck = n_survive * 35000.0
        ...
        return DSSSimulationResponse(
            ...
            N_survive=round(n_survive, 4),
            Yield_are_predict=round(yield_are_predict, 4),
            Yield_total_predict=round(yield_total_predict, 4),
            ...
            Cost_labor_total=round(cost_labor_total, 2),
            Cost_infra=round(cost_infra, 2),
