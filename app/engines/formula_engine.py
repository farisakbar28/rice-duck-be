"""A+C dual-evidence formula engines.

The C0 production path and the Xiong literature path intentionally have
separate functions so a reference value cannot enter production economics.
"""
...
Y0_C = Decimal("50.0")
P_GABAH_DEFAULT = Decimal("6000")
P_DUCK_BUY_DEFAULT = Decimal("25000")
P_DUCK_SELL_DEFAULT = Decimal("45000")
...
def compute_primary_yield(land_area_are: object) -> dict:
    """Frozen local C0 production yield; it accepts no biological modifiers."""
    return {"yield_are_kg": Y0_C, "yield_total_kg": Y0_C * decimal(land_area_are)}


def compute_literature_reference(density_ha: Decimal, duration_days: object | None, land_area_are: object) -> dict:
    """Evaluate Xiong exactly inside its published domain, otherwise abstain."""
    if duration_days is None:
        return {"status": "OUTSIDE_LITERATURE_DOMAIN", "yield_are_kg": None, "yield_total_kg": None, "reason": "literature_duration_days was not supplied"}
    t = decimal(duration_days)
    if not (Decimal("0") < density_ha <= Decimal("600") and Decimal("50") <= t <= Decimal("80")):
        return {"status": "OUTSIDE_LITERATURE_DOMAIN", "yield_are_kg": None, "yield_total_kg": None, "reason": "Xiong requires 0 < density_ha <= 600 and 50 <= literature_duration_days <= 80"}
...
def compute_economics_from_primary(*, primary_total_kg: Decimal, duck_count: int, density_are: Decimal, p_gabah: object, p_duck_buy: object, p_duck_sell: object, c_feed_scenario: object | None, c_jaring_purchase: object | None, n_jaring_cycles: object | None, c_kandang_purchase: object | None, n_kandang_cycles: object | None) -> dict:
    """Scenario cash arithmetic sourced exclusively from the C0 primary yield."""
    revenue_gabah = primary_total_kg * decimal(p_gabah)
    ...
    before = revenue_gabah + duck_revenue - cost_buy if duck_revenue is not None else None
    infra, infra_selected = Decimal("0"), False
    if c_jaring_purchase is not None:
        infra += decimal(c_jaring_purchase) / decimal(n_jaring_cycles); infra_selected = True
    if c_kandang_purchase is not None:
        infra += decimal(c_kandang_purchase) / decimal(n_kandang_cycles); infra_selected = True
