...
### Added
- Closed the current R2 Phase-6 documentation release with the active
  ...
  semantics, corrected retrospective comparator evidence, and publication
  export sources. The release status is
  `R2_PHASE6_TECHNICAL_EMPIRICAL_RELEASE_CLOSED_WITH_LIMITATIONS`.
...
- Added Phase-5 freeze governance metadata: `MODEL_FROZEN`, `FREEZE_ID`
  (historical Phase-5 value `R2-FREEZE-2026-08-26.1`), and
  `FREEZE_EFFECTIVE_FROM` in the seed layer,
...
- Added `docs/14_R2_FINAL_RELEASE_CLOSURE.md` and the `docs/export/` source
  package for later mathematical-model, validation-methodology, and IJoST
  artifact generation. No DOCX is generated by this release-closure change.
- Recorded Phase 6E approval with limitations and the non-blocking status
...
- Active yield lookup now normalizes approved input labels to the external
  cultivar-group baselines; availability requires a resolved group baseline,
  supported age/density, and the pooled `F_RD_ref`. Unresolved or out-of-domain
  requests fail closed, and the group records do not claim exact local-cultivar
  or Bali calibration.
...
- Current release identity is `MODEL_VERSION=R2`, registry
  `R2-2026-08-26.3`, freeze `R2-FREEZE-2026-08-26.5`; intermediate `.1`-`.4`
  freeze values remain historical lineage; registry `.3` is the current runtime
  registry and freeze `.5` is the current validation target.
- Corrected F-01/F-02/F-03 documentation and validation provenance is retained
  as an auditable correction history; the frozen scientific model was not
  recalibrated.
