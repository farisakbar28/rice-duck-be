...
H=[
 {'id':'H01','raw_row':8,'area':3.60,'J':13,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':45.83333333,'actual_gabah_revenue':990000,'p_gabah':6000,'p_duck_buy':25000},
 {'id':'H02','raw_row':9,'area':5.10,'J':5,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':48.03921569,'actual_gabah_revenue':1470000,'p_gabah':6000,'p_duck_buy':25000},
 {'id':'H03','raw_row':11,'area':10,'J':65,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':60.5,'actual_gabah_revenue':3630000,'p_gabah':6000,'p_duck_buy':7539},
 {'id':'H04','raw_row':14,'area':7.26,'J':9,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':59.36639118,'actual_gabah_revenue':3232500,'p_gabah':7500,'p_duck_buy':22222.22222},
 {'id':'H05','raw_row':23,'area':5.10,'J':10,'variety':'inpari','planting_system':'jajar_legowo','actual_yield':21.01960784,'actual_gabah_revenue':804000,'p_gabah':7500,'p_duck_buy':5000},
 {'id':'H06','raw_row':25,'area':14.41,'J':30,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':52.42886884,'actual_gabah_revenue':5666250,'p_gabah':7500,'p_duck_buy':10000},
 {'id':'H07','raw_row':38,'area':10,'J':32,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':53.4,'actual_gabah_revenue':3364200,'p_gabah':6300,'p_duck_buy':0,'planting_date':'2024-04-22'},
 {'id':'H08','raw_row':43,'area':3.60,'J':15,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':40.41666667,'actual_gabah_revenue':873000,'p_gabah':6000,'p_duck_buy':0,'planting_date':'2024-10-01'},
 {'id':'H09','raw_row':44,'area':10,'J':29,'variety':'inpari','planting_system':'tegel','actual_yield':38.65,'actual_gabah_revenue':2319000,'p_gabah':6000,'p_duck_buy':0,'planting_date':'2024-09-28'},
 {'id':'H10','raw_row':47,'area':3,'J':6,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':13.5,'actual_gabah_revenue':243000,'p_gabah':6000,'p_duck_buy':25000},
 {'id':'H11','raw_row':62,'area':3.77,'J':8,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':36.47214854,'actual_gabah_revenue':825000,'p_gabah':6000,'p_duck_buy':25000},
]
...
HOLDOUT_ACTUAL_SOURCE={'file':'DSS_Padi_Bebek_Rekap_Bersih_v10(1).xlsx','sheet':'Dataset Actual Bersih','join_key':'Excel Row (Sumber)','actual_yield_field':'Actual Yield (kg/are)','actual_gabah_revenue_field':'Actual Gabah Revenue (Rp)'}
...
def main():
 ...
 try:
  ...
  rows=[]
  for row in H:
   ident,raw_row,a,j,v,s,actual,actual_gabah_revenue,pg,pb=(row[key] for key in ('id','raw_row','area','J','variety','planting_system','actual_yield','actual_gabah_revenue','p_gabah','p_duck_buy'))
   ...
   e=call(base,'/api/v1/dss/simulate',request); b=e['response']; err=b['yield_are_kg']-actual; backend_revenue_expected=50*a*pg; expected_cost=j*pb; expected_cash=backend_revenue_expected+j*45000-expected_cost; revenue_ok=abs(b['revenue_gabah']-backend_revenue_expected)<0.001; cost_ok=abs(b['cost_duck_buy']-expected_cost)<=.01; cash_ok=abs(b['cash_contribution_before_optional']-expected_cash)<=.01; price_runtime=b['provenance']['prices']['p_duck_buy']['source']=='runtime' and b['provenance']['prices']['p_duck_buy']['status']=='runtime'; calendar_ok=(all(b[key] is None for key in ('release_date_min','release_date_max','withdraw_date_min','withdraw_date_max')) if planting_date is None else all(b[key] is not None for key in ('release_date_min','release_date_max','withdraw_date_min','withdraw_date_max'))); e.update({'id':ident,'raw_row':raw_row,'source_planting_date':planting_date,'actual_yield':actual,'actual_gabah_revenue':actual_gabah_revenue,'backend_yield':b['yield_are_kg'],'error':err,'density_expected':j/a,'density_returned':b['density_are'],'density_status':b['density_status'],'backend_revenue_gabah_expected':backend_revenue_expected,'backend_revenue_gabah':b['revenue_gabah'],'backend_revenue_gabah_check':revenue_ok,'actual_gabah_revenue_comparison':{'source_value':actual_gabah_revenue,'backend_value':b['revenue_gabah'],'backend_minus_actual':b['revenue_gabah']-actual_gabah_revenue},'p_duck_buy_source':pb,'cost_duck_buy_expected':expected_cost,'cost_duck_buy_backend':b['cost_duck_buy'],'cost_duck_buy_check':cost_ok,'cash_contribution_expected':expected_cash,'cash_contribution_backend':b['cash_contribution_before_optional'],'cash_contribution_check':cash_ok,'p_duck_buy_provenance_runtime':price_runtime,'calendar_window':{key:b[key] for key in ('release_date_min','release_date_max','withdraw_date_min','withdraw_date_max')},'calendar_check':calendar_ok,'pass':e['http_status']==200 and b['yield_are_kg']==50 and abs(b['density_are']-j/a)<1e-9 and revenue_ok and cost_ok and cash_ok and price_runtime and calendar_ok}); rows.append(e)
  ...
  frozen_metrics={'MAE':11.9785716318,'RMSE':15.9898352553,'MedAE':9.5833333300,'Bias':7.3067061736}
  metrics_pass=all(abs(metrics[key]-expected)<=1e-9 for key,expected in frozen_metrics.items())
  ...
  after=snap(main_db); cost_pass=sum(x['cost_duck_buy_check'] for x in rows); cash_pass=sum(x['cash_contribution_check'] for x in rows); calendar_holdout_pass=sum(x['calendar_check'] for x in rows); evidence={'branch':branch,'base_head':base_head,'working_tree_dirty':working_tree_dirty,'captured_at':datetime.now(timezone.utc).isoformat(),'server_url':base,'runtime_instance_id':nonce,'backend_start_command':'python -m uvicorn app.main:app','runtime_database_path':str(db),'main_database_before':before,'main_database_after':after,'main_database_unchanged':before==after,'health':health,'holdout_actual_source':HOLDOUT_ACTUAL_SOURCE,'holdout_p_duck_buy_source':HOLDOUT_P_DUCK_BUY_SOURCE,'holdout_planting_date_source':HOLDOUT_PLANTING_DATE_SOURCE,'holdout':rows,'holdout_metrics':metrics,'frozen_holdout_metrics':frozen_metrics,'academic_displayed_holdout_metrics':{'MAE':11.979,'RMSE':15.990,'MedAE':9.583,'Bias':'+7.307'},'synthetic':synt,'calendar':cal,'history':history,'summary':{'holdout_pass':sum(x['pass'] for x in rows),'holdout_total':11,'holdout_metrics_pass':metrics_pass,'holdout_cost_duck_buy_audit_pass':cost_pass,'holdout_cost_duck_buy_audit_total':11,'holdout_cash_contribution_audit_pass':cash_pass,'holdout_cash_contribution_audit_total':11,'holdout_calendar_audit_pass':calendar_holdout_pass,'holdout_calendar_audit_total':11,'synthetic_pass':sum(x['pass'] for x in synt),'synthetic_total':12,'calendar_pass':cal['pass'],'history_pass':history['pass']}}
