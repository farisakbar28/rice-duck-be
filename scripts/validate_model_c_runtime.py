...
H=[
 ...
 {'id':'H07','raw_row':38,'area':10,'J':32,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':53.40,'p_gabah':6300,'p_duck_buy':0},
 {'id':'H08','raw_row':43,'area':3.60,'J':15,'variety':'sertani','planting_system':'jajar_legowo','actual_yield':40.42,'p_gabah':6000,'p_duck_buy':0},
 {'id':'H09','raw_row':44,'area':10,'J':29,'variety':'inpari','planting_system':'tegel','actual_yield':38.65,'p_gabah':6000,'p_duck_buy':0},
...
def main():
 ...
 try:
  ...
  after=snap(main_db); cost_pass=sum(x['cost_duck_buy_check'] for x in rows); cash_pass=sum(x['cash_contribution_check'] for x in rows); evidence={'branch':branch,'base_head':base_head,'working_tree_dirty':working_tree_dirty,'captured_at':datetime.now(timezone.utc).isoformat(),'server_url':base,'runtime_instance_id':nonce,'backend_start_command':'python -m uvicorn app.main:app','runtime_database_path':str(db),'main_database_before':before,'main_database_after':after,'main_database_unchanged':before==after,'health':health,'holdout_p_duck_buy_source':HOLDOUT_P_DUCK_BUY_SOURCE,'holdout':rows,'holdout_metrics':metrics,'frozen_holdout_metrics':frozen_metrics,'synthetic':synt,'calendar':cal,'history':history,'summary':{'holdout_pass':sum(x['pass'] for x in rows),'holdout_total':11,'holdout_metrics_pass':metrics_pass,'holdout_cost_duck_buy_audit_pass':cost_pass,'holdout_cost_duck_buy_audit_total':11,'holdout_cash_contribution_audit_pass':cash_pass,'holdout_cash_contribution_audit_total':11,'synthetic_pass':sum(x['pass'] for x in synt),'synthetic_total':12,'calendar_pass':cal['pass'],'history_pass':history['pass']}}
  OUT.write_text(json.dumps(evidence,ensure_ascii=False,indent=2)); print(json.dumps(evidence['summary'])); return 0 if evidence['summary']['holdout_pass']==11 and metrics_pass and cost_pass==11 and cash_pass==11 and evidence['summary']['synthetic_pass']==12 and cal['pass'] and history['pass'] else 1
 finally:
